...
# XML 1.0 forbids most control characters — the LLM occasionally emits them.
_XML_BAD = re.compile(r'[\x00-\x08\x0b\x0c\x0e-\x1f]')


def _sanitize(obj):
    """Recursively strip XML-invalid control characters from every string."""
    if isinstance(obj, str):
        return _XML_BAD.sub('', obj)
...
def export_to_docx(
    ...
    """
    # Right-to-left layout + Arabic styling for EVERY non-English subject.
    # Only an explicitly-detected English subject stays left-to-right (LTR).
    # (So Arabic, physics-in-Arabic, history, etc. all align right; English aligns left.)
    is_arabic = (language or "en").strip().lower() != "en"
    questions = _sanitize(questions)        # remove XML-invalid control chars from LLM output
    doc = Document()
    ...
    for q_type in _TYPE_ORDER:
        ...
        # Questions (defensive: one malformed question never kills the whole file)
        for i, q in enumerate(valid_qs, 1):
            try:
                _render_question_item(doc, q, i, q_type, is_arabic)
            except Exception as e:
                import sys
                print(f"[docx] skipped {q_type} Q{i}: {e}", file=sys.stderr)
    ...
    if include_answers and grouped:
        try:
            ...
            import sys
            print(f"[docx] answer key error: {e}", file=sys.stderr)
