...
class _TeacherChatScreenState extends State<TeacherChatScreen> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // ── Messages list ──────────────────────────────────────────────
          Expanded(
            child: BlocConsumer<TeacherChatCubit, TeacherChatState>(
              listener: (context, state) {
                if (state is TeacherChatLoaded) {
                  _scrollToBottom();
                }
              },
              builder: (context, state) {
                if (state is TeacherChatLoading) {
                  return const Center(child: CircularProgressIndicator());
                }
                if (state is TeacherChatError) {
                  return Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        TextButton(
                          onPressed: () =>
                              context.read<TeacherChatCubit>().load(),
  ...
  Future<void> _send(BuildContext context) async {
    ...
    _ctrl.clear();
    final ok = await context.read<TeacherChatCubit>().sendReply(body);
...
class _InputBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<TeacherChatCubit, TeacherChatState>(
      builder: (context, state) {
        final sending = state is TeacherChatLoaded && state.sending;
