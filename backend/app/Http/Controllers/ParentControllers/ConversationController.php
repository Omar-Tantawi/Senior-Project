...
{
    /**
     * List all conversations for this parent, most recent first.
     */
    public function index(int $parentId)
    {
        ...
        $conversations = Conversation::where('parent_user_id', $userId)
            ->with(['teacherUser', 'lastMessage'])
    ...
    /**
     * Start a new conversation (or reuse an existing one) with a teacher.
     *
     * Body: { teacher_id: int, body: string }
     *   teacher_id is the Teacher model id (not user_id).
     */
    public function start(int $parentId, Request $request)
    {
        $guardian = Guardian::findOrFail($parentId);
        $userId   = $guardian->user_id;
        ...
        $conversation = Conversation::firstOrCreate(
            ['teacher_user_id' => $teacher->user_id, 'parent_user_id' => $userId]
        );
     ...
     * Accepts ?after_id=N for polling (only messages with id > N).
     * Marks all incoming messages as read automatically.
     */
    public function messages(int $parentId, int $convId, Request $request)
    {
        $guardian = Guardian::findOrFail($parentId);
        $userId   = $guardian->user_id;
     ...
     * Body: { body: string }
     */
    public function reply(int $parentId, int $convId, Request $request)
    {
        $guardian = Guardian::findOrFail($parentId);
        $userId   = $guardian->user_id;
        ...
        $message = Message::create([
            ...
            'sender_id'       => $userId,
            'receiver_id'     => $conv->teacher_user_id,
            'body'            => $request->body,
        ]);
     ...
     */
    public function markRead(int $parentId, int $convId)
    {
        $guardian = Guardian::findOrFail($parentId);
        $userId   = $guardian->user_id;
